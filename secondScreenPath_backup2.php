...
<body>
    <div class="container mt-4">
        ...
        <table id="table" class="table table-striped" 
            ...
            data-filter-control="true"> <!-- Enable filter control -->
            <thead>
                <tr>
                    ...
                    <!-- <th data-field="scm-status" data-sortable="true" data-filter-control="select">SCM Status</th> -->

                    <th data-field="scm-status" data-sortable="true" data-filter-control="select" data-filter-data="var:scmStatusOptions">SCM Status</th>
            ...
            <tbody>
                ...
                <tr data-record='<?php echo json_encode([
                    ...
                    <td data-value="<?= !empty($item['module_record_id']) ? $item['module_record_id'] : '' ?>">
                        <?= !empty($item['cf_po_num']) ? $item['cf_po_num'] : 'N/A' ?>
                    </td>
                    ...
                    <td>
                        <select class="form-select scm-status" data-record-id="<?= $item['module_record_id'] ?>" name="scm_head_status">
                            <option value="" selected disabled> <?= !empty($item['cf_scm_head_approval_status']) && $item['cf_scm_head_approval_status'] !== 'Approve' 
                        ...
                        <select class="form-select finance-status" data-record-id="<?= $item['module_record_id'] ?>" name="finance_head_status">
                            <option value="" selected disabled>
                                <?= !empty($item['cf_finance_head_approval_statu']) && $item['cf_finance_head_approval_statu'] !== 'Approve' 
                                    ? htmlspecialchars($item['cf_finance_head_approval_statu']) : 'Select Status' ?>
                            </option>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('.bootstrap-table-filter-control-scm-status').on('change', function () {
        let selectedStatus = $(this).val(); // Get selected status
        console.log("selectedStatus: " );

        $('#your-table tbody tr').each(function () {
            let rowStatus = $(this).find('td[data-field="scm-status"]').text().trim();
        ...
        // Initialize Bootstrap Table with Filter Control
        ...
        // Your existing JavaScript code for handling approvals
        ...
        function processApproval(formData, type) {
            $.ajax({
                ...
                type: 'GET',
                data: { module_record_id: pi_Id },
                success: function (response) {
                    const piData = JSON.parse(response);
                    const moduleRecord = JSON.parse(piData).module_record.module_fields;
                    const result = moduleRecord.find(item => item.api_name === "cf_cm_approve_status_history");
                    ...
                    $.ajax({
                        ...
                        error: function (xhr, status, error) {
                            alert(`Failed to update ${type} details. Please try again.`);
                        }
                    });
                },
                error: function (xhr, status, error) {
